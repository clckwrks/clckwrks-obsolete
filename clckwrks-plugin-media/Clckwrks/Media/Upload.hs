...
isSupportedType :: String -> Bool
isSupportedType ct = (takeWhile (/= ';') ct) `elem` acceptedTypes
      ...
      saveMedia (Just (origName, tempPath)) =
          do md <- mediaDirectory <$> ask
             ...
             case contentTypeExtension contentType of
                ...
                (Just (ext, kind)) ->
                    do -- liftIO $ putStrLn contentType -- "image/jpeg; charset=binary"

                       -- renameFile would be faster, but may not work if it has to cross physical devices
